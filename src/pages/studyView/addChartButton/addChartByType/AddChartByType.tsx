...
> {
    ...
    render() {
        let options = this.options.map(item => {
            if (item.label === "Cancer Studies")
                item.label = "Studies"

            if (item.label === "Number of Samples Per Patient")
                item.label = "Number of Samples"

            if (item.label === "# Samples per Patient")
                item.label = "# Samples per Individual"                

            return item;
        });
        options = options.filter(item => {
            if (item.label === "Overall Survival Status")
                return false;

            if (item.label === "Disease Free (Months)")
                return false;

            if (item.label === "Disease Free Status")
                return false;

            if (item.label === "Overall Survival (Months)")
                return false;
