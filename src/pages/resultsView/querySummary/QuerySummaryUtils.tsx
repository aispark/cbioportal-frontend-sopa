...
export function getPatientSampleSummary(samples: any[], patients: any[]) {
    if (samples.length !== patients.length) {
        return (
            <span>
                <strong>{patients.length}</strong> individuals /{' '}
        ...
        return (
            <span>
                <strong>{samples.length}</strong> individuals/samples
            </span>
        );
    }
}
...
export function getAlterationSummary(
    ...
    if (numSamples !== numPatients) {
        ...
        return (
            <strong>
                <span>{`${prefix} ${sampleSummaryPrefix} individuals/samples`}</span>
            </strong>
        );
    }
}
